      ...
      def perform(wallet)
        ...
        data_min['rigs'].each do |rig|
          ...
          shares = { accepted: accepted_shares.sum, staled: staled_shares.sum, duplicated: staled_duplicated.sum }
                   .map { |_k, v| just(b(v), 7) }.join(' | ')
          worker = "#{rig['name']}:"
          text << worker
          text << "#{b(to_gh(rig_hashrate).round(2))} Gh/s -> #{shares}"
          text << '=========='
        end
        ...
        text << "Miner balance: #{b(balance)} tons"
        ...
        text << '<b>NFT NG</b> — Уникальные цифровые активы от ваших любимых разработчиков <u>toncoinpool.io</u>.'
        text << '<u>@nftngio</u> - следите за новостями! ❤️‍🔥'
